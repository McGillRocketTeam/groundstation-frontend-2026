...
const MapCardConfiguration = Schema.TaggedStruct("MapCard", {
  ...
  long: Schema.Number
    ...
    (),
  tempTrackerLong: Schema.Number
    .pipe
    // Schema.greaterThan(-180),
    // Schema.lessThan(180),
    // Schema.annotations({ title: "Marker Longitude" }),
    (),
  tempTrackerLat: Schema.Number
    .pipe
    // Schema.greaterThan(-90),
    // Schema.lessThan(90),
  ...
  trackerLat: Schema.optional(ParameterInfoType).annotations({
    title: "Tracker Latitude",
  }),
  // tracker longitude remains optional (user may choose not to select it)
  trackerLong: Schema.optional(ParameterInfoType).annotations({
    title: "Tracker Longitude",
  }),
